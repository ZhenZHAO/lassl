...
class TwoCropsTransform:
    """generaate 2 random augmentations of one image."""
...
def load_data_train(L=250, dataset='CIFAR10', dspth='./data'):
    if dataset == 'CIFAR10':
        ...
        n_class = 10
        assert L in [10, 20, 40, 80, 100, 250, 1000, 4000]
    elif dataset == 'CIFAR100':
        datalist = [
            osp.join(dspth, 'cifar-100-python', 'train')]
        n_class = 100
        assert L in [400, 1000, 2500, 10000]
    ...
    for i in range(n_class):
        ...
        lb_index.extend(inds_x.tolist())
    file_save_labels = f'labels_index_cifar_{L}.txt'
#     with open('labels_index_cifar.txt', mode='a') as filename:
    with open(file_save_labels, mode='a') as filename:
        filename.write("="*100)
        filename.write('\n')
        filename.write(",".join([str(ss) for ss in lb_index]))
        filename.write('\n') 
...
class Cifar(Dataset):
    def __init__(self, dataset, data, labels, mode):
        ...
        if self.mode == 'weak':
            ...
            self.trans = TwoCropsTransform(trans_weak, trans_strong1)
        elif self.mode == 'semi':
            self.trans = TwoCropsTransform(trans_weak, trans_strong0)
        elif self.mode == 'self_weak':
            self.trans = TwoCropsTransform(trans_weak, trans_weak)
        else:
...
def get_train_loader(dataset, batch_size, mu, n_iters_per_epoch, L, num_workers, root='data', method='self', unmethod="semi"):
